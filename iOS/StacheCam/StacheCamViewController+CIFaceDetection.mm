...
- (void) setupCoreImageFaceDetection
{
    ...
    self.recognizer = emoRecognizer();
    self.outputSampleCounter = 0;
}
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    BOOL processFrame = self.videoRecordingStatus == VideoRecordingStatusRecording && validFaces && self.outputSampleCounter % 2 == 0;
    
    if (processFrame) {
        [self.movieRecorder appendVideoSampleBuffer:sampleBuffer];
        [self recordFrameForFeatures:features withCIImage:ciImage];
    }
...
- (void) recordFrameForFeatures:(NSArray *)features withCIImage:(CIImage *)ciImage
{
    NSMutableArray* faces = [[NSMutableArray alloc] init];
    std::vector<emoFace> emoFaces = std::vector<emoFace>();
    for (CIFaceFeature *ff in features) {
        Face *face = [[Face alloc] initWithBounds:ff.bounds
                                  leftEyePosition:ff.leftEyePosition
                                 rightEyePosition:ff.rightEyePosition];
        [faces addObject:face];

        CGRect bounds = ff.bounds;
        emoFace eFace = emoFace();
        eFace.boundingBox = cv::Rect(bounds.origin.x, bounds.origin.y, bounds.size.width, bounds.size.height);
        eFace.leftEye = cv::Point(ff.leftEyePosition.x, ff.leftEyePosition.y);
        eFace.rightEye = cv::Point(ff.rightEyePosition.x, ff.rightEyePosition.y);
        emoFaces.push_back(eFace);
    }
    if (emoFaces.size() > 0) {
        NSDate *currentTime = [NSDate new];
        long timestamp = [currentTime timeIntervalSinceDate:self.startTime] * 1000;
        
        UIImage *uiImage = [[UIImage alloc] initWithCIImage:ciImage];

        self.recognizer.process([uiImage CVMat], emoFaces, timestamp);
        
        int emotion = emoFaces.at(0).emotion;
        NSLog(@"Emotion: %d", emotion);
        
        Frame *frame = [[Frame alloc] initWithTimestamp:timestamp
                                                  faces:faces
                                                emotion:emotion];
...
- (void) drawFaceBoxesForFeatures:(NSArray *)features forVideoBox:(CGRect)clap orientation:(UIDeviceOrientation)orientation
{
    ...
    NSString *recording = self.videoRecordingStatus == VideoRecordingStatusRecording ? @"recording" : @"";
	if ( self.ciFaceLayers.count == 0 ) {
		self.ciLastFrameTime = [NSDate date];
	} else if ( features.count == 0 ) {
		self.ciFPSLabel.text = [NSString stringWithFormat:@"CI FPS: - %@", recording];
	} else {
		NSDate* curTime = [NSDate date];
		self.ciProcessingInterval = self.ciProcessingInterval*0.75 + [curTime timeIntervalSinceDate:self.ciLastFrameTime]*0.25;
        self.ciFPSLabel.text = [NSString stringWithFormat:@"CI FPS: % 3.0f - %@",1/self.ciProcessingInterval, recording];
	...
	for ( CIFaceFeature *ff in features ) {
        ...
        CGRect faceRect = [StacheCamViewController adjustedRectForRect:[ff bounds] relativeToPreviewBox:previewBox forVideoBox:clap];
...
+ (CGRect) adjustedRectForRect:(CGRect)originalRect relativeToPreviewBox:(CGRect)previewBox forVideoBox:(CGRect)videoBox
{
    // flip preview width and height
    CGRect returnedRect = CGRectMake(originalRect.origin.y, originalRect.origin.x,
                                     originalRect.size.height, originalRect.size.width);
    ...
    returnedRect.size.width *= widthScaleBy;
    returnedRect.size.height *= heightScaleBy;
    returnedRect.origin.x *= widthScaleBy;
    returnedRect.origin.y *= heightScaleBy;
    return returnedRect;
}
